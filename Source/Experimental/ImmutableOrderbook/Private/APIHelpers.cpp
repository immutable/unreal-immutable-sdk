...
void HttpMultipartFormData::SetupHttpRequest(const FHttpRequestRef& HttpRequest)
{
	...
	{
		UE_LOG(LogImmutableOrderbook, Error, TEXT("Expected POST verb when using multipart form data"));
	}
...
void HttpMultipartFormData::AddFilePart(const TCHAR* Name, const HttpFileInput& File)
{
	...
	{
		UE_LOG(LogImmutableOrderbook, Error, TEXT("Failed to load file (%s)"), *File.GetFilePath());
		return;
	}
