...
UImmutablePassportImxBatchNftTransferAsyncAction::ImxBatchNftTransfer(
    UObject *WorldContextObject,
    const TArray<FNftTransferDetails> &NftTransferDetails) {
  UImmutablePassportImxBatchNftTransferAsyncAction *BlueprintNode =
      NewObject<UImmutablePassportImxBatchNftTransferAsyncAction>();
...
void UImmutablePassportImxBatchNftTransferAsyncAction::Activate() {
  if (!WorldContextObject || !WorldContextObject->GetWorld()) {
    const FString Err =
        "BatchNftTransfer failed due to missing world or world context object.";
  ...
  GetSubsystem()->WhenReady(
      this, &UImmutablePassportImxBatchNftTransferAsyncAction::DoTransfer);
}

void UImmutablePassportImxBatchNftTransferAsyncAction::DoTransfer(
  ...
  // Run Transfer
  GetSubsystem()->GetPassport()->ImxBatchNftTransfer(
      Request,
      UImmutablePassport::FImtblPassportResponseDelegate::CreateUObject(
          this, &UImmutablePassportImxBatchNftTransferAsyncAction::
                    OnTransferResponse));
}

void UImmutablePassportImxBatchNftTransferAsyncAction::OnTransferResponse(
    FImmutablePassportResult Result) {
  TArray<FString> TransferIds;
  if (Result.Success) {
    if (auto BatchNftTransferResponse =
            JsonObjectToUStruct<FImxBatchNftTransferResponse>(
