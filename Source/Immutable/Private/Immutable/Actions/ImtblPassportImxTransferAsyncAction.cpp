...
UImmutablePassportImxTransferAsyncAction::ImxTransfer(
    UObject *WorldContextObject, const FString &Receiver, const FString &Type,
    const FString &Amount, const FString &TokenId,
    const FString &TokenAddress) {
  UImmutablePassportImxTransferAsyncAction *BlueprintNode =
      NewObject<UImmutablePassportImxTransferAsyncAction>();
...
void UImmutablePassportImxTransferAsyncAction::Activate() {
  if (!WorldContextObject || !WorldContextObject->GetWorld()) {
    const FString Err =
        "Transfer failed due to missing world or world context object.";
  ...
  GetSubsystem()->WhenReady(
      this, &UImmutablePassportImxTransferAsyncAction::DoTransfer);
}

void UImmutablePassportImxTransferAsyncAction::DoTransfer(
  ...
  // Run Transfer
  GetSubsystem()->GetPassport()->ImxTransfer(
      Request,
      UImmutablePassport::FImtblPassportResponseDelegate::CreateUObject(
          this, &UImmutablePassportImxTransferAsyncAction::OnTransferResponse));
}

void UImmutablePassportImxTransferAsyncAction::OnTransferResponse(
    FImmutablePassportResult Result) {
  if (Result.Success) {
    FString TransferId;
    if (auto TransferResponse = JsonObjectToUStruct<FImxTransferResponse>(
