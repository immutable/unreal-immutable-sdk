...
UImtblPassportZkEvmSendTransactionAsyncAction::ZkEvmSendTransaction(
    UObject *WorldContextObject, const FImtblTransactionRequest &Request) {
  UImtblPassportZkEvmSendTransactionAsyncAction
      *PassportZkEvmSendTransactionBlueprintNode =
          NewObject<UImtblPassportZkEvmSendTransactionAsyncAction>();
  PassportZkEvmSendTransactionBlueprintNode->WorldContextObject =
      WorldContextObject;
...
void UImtblPassportZkEvmSendTransactionAsyncAction::Activate() {
  if (!WorldContextObject || !WorldContextObject->GetWorld()) {
    FString Err = "ZkEvmSendTransaction failed due to missing world or world "
                  "context object.";
    IMTBL_WARN("%s", *Err)
  ...
  GetSubsystem()->WhenReady(
      this,
      &UImtblPassportZkEvmSendTransactionAsyncAction::DoZkEvmSendTransaction);
}

void UImtblPassportZkEvmSendTransactionAsyncAction::DoZkEvmSendTransaction(
  ...
  Passport->ZkEvmSendTransaction(
      ...
      UImmutablePassport::FImtblPassportResponseDelegate::CreateUObject(
          this, &UImtblPassportZkEvmSendTransactionAsyncAction::
                    OnZkEvmSendTransactionResponse));
}

void UImtblPassportZkEvmSendTransactionAsyncAction::
    OnZkEvmSendTransactionResponse(FImmutablePassportResult Result) {
  if (Result.Success) {
