...
UImtblBrowserWidget::UImtblBrowserWidget() {
  ...
  JSConnector->ExecuteJs =
      UImtblJSConnector::FOnExecuteJsDelegate::CreateUObject(
          this, &UImtblBrowserWidget::ExecuteJS);
...
void UImtblBrowserWidget::SetBrowserContent() {
  ...
  if (UObject *LoadedAsset = AssetRef.TryLoad()) {
    if (auto Resource = Cast<UImtblSDKResource>(LoadedAsset)) {
      ...
      const FString IndexHtml =
          FString("<!doctype html><html lang='en'><head><meta "
                  "charset='utf-8'><title>GameSDK Bridge</title><script>") +
          Resource->Js +
          FString(
              "</script></head><body><h1>Bridge Running</h1></body></html>");

      // IMTBL_LOG("Loaded resource: %s", *Resource->GetName())
      WebBrowserWidget->LoadString(IndexHtml,
                                   TEXT("file://immutable/index.html"));
...
bool UImtblBrowserWidget::BindUObject(const FString &Name, UObject *Object,
                                      const bool bIsPermanent) const {
#if USING_BUNDLED_CEF
  if (!WebBrowserWidget) {
    IMTBL_WARN_FUNC(
        "Could not bind UObject '%s' to browser, WebBrowserWidget is null",
        *Object->GetName())
    return false;
  }
  ...
  if (IsDesignTime()) {
    ...
    WebBrowserWidget =
        SNew(SWebBrowser)
            ...
            .OnConsoleMessage(BIND_UOBJECT_DELEGATE(FOnConsoleMessageDelegate,
                                                    HandleOnConsoleMessage))
#endif
        ;
...
void UImtblBrowserWidget::HandleOnConsoleMessage(
  ...
  IMTBL_LOG("Browser console message: %s, Source: %s, Line: %d", *Message,
            *Source, Line);
