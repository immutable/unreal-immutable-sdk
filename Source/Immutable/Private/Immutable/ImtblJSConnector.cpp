...
void UImtblJSConnector::Init(bool bPageLoaded) {
  ...
  if (bPageLoaded) {
    IMTBL_LOG("Browser finished loading the bridge document before the "
              "Immutable JSConnector was created & bound")
    ...
    // message
    RequestResponseDelegates.Add(
        "1", FImtblJSResponseDelegate::CreateUObject(
                 this, &UImtblJSConnector::HandleInitResponse));
  }
}
...
void UImtblJSConnector::AddCallbackWhenBridgeReady(
    const FOnBridgeReadyDelegate::FDelegate &Delegate) {
  if (IsBridgeReady()) {
    Delegate.Execute();
  } else {
...
FString
UImtblJSConnector::CallJS(const FString &Function, const FString &Data,
                          const FImtblJSResponseDelegate &HandleResponse,
                          const float ResponseTimeout) {
  const FString Guid = FGuid::NewGuid().ToString();
  CallJS(FImtblJSRequest{Function, Data, Guid}, HandleResponse,
         ResponseTimeout);
  return Guid;
}

void UImtblJSConnector::CallJS(const FImtblJSRequest &Request,
  ...
  if (!IsBridgeReady()) {
    ...
    AddCallbackWhenBridgeReady(FOnBridgeReadyDelegate::FDelegate::CreateLambda(
        [Request, HandleResponse, ResponseTimeout, this]() {
  ...
  RequestString.ReplaceInline(TEXT("\""), TEXT("\\\""));
  const FString JS =
      FString::Printf(TEXT("callFunction(\"%s\");"), *RequestString);
...
void UImtblJSConnector::SendToGame(FString Message) {
  ...
  const TOptional<FImtblJSResponse> Response =
      FImtblJSResponse::FromJsonString(Message);
  if (!Response.IsSet()) {
  ...
  if (!RequestResponseDelegates.Contains(Response->requestId)) {
    IMTBL_WARN("No delegate found for response with id %s",
               *Response->requestId);
    return;
  }

  if (!RequestResponseDelegates[Response->requestId].ExecuteIfBound(
          Response.GetValue())) {
    IMTBL_WARN("Delegate for response with id %s failed to execute",
               *Response->requestId);
  }
