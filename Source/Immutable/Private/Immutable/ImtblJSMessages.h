...
struct FImtblResponseError {
  ...
  FString ToString() {
    const FString ErrType =
        StaticEnum<EImtblPassportError>()->GetNameStringByValue(
            static_cast<int8>(errorType));
...
struct FImtblJSRequest {
  ...
  FString ToJsonString() const {
    FString OutString;
    FJsonObjectConverter::UStructToJsonObjectString<FImtblJSRequest>(
        *this, OutString, 0, 0, 0, nullptr, false);
    return OutString;
  }
};
...
struct IMMUTABLE_API FImtblJSResponse {
  ...
  static TOptional<FImtblJSResponse> FromJsonString(const FString &JsonString) {
    ...
    if (!FJsonSerializer::Deserialize(JsonReader, JSResponse.JsonObject) ||
        !JSResponse.JsonObject.IsValid()) {
      IMTBL_ERR("Could not parse response from JavaScript -- invalid JSON: %s",
                *JsonString)
    } else if (!FJsonObjectConverter::JsonObjectToUStruct(
                   JSResponse.JsonObject.ToSharedRef(), &JSResponse, 0, 0)) {
    ...
    if (!JSResponse.success) {
      ...
      JSResponse.Error =
          FImtblResponseError{static_cast<EImtblPassportError>(ErrType), Error};
    }
