...
// Disable for shipping builds
    ...
    IMMUTABLE_API extern struct FLogCategoryLogClean : public FLogCategory<ELogVerbosity::Log, ELogVerbosity::All> { FORCEINLINE FLogCategoryLogClean() : FLogCategory(TEXT("")) {} } LogClean;
#endif

#define IMTBL_LOG_CLEAN(Format, ...) UE_LOG(LogClean, Log, TEXT(Format), ##__VA_ARGS__)
...
#define IMTBL_LOG_FUNCSIG UE_LOG(LogImmutable, Log, TEXT("%s"), *FString(IMTBL_FUNCSIG))
...
{ \
	const FString _Msg = FString::Printf(TEXT(Format), ##__VA_ARGS__); \
	UE_LOG(LogImmutable, Verbose, TEXT("%s: %s"), *FString(IMTBL_FUNCSIG), *_Msg) \
}
...
{ \
	const FString _Msg = FString::Printf(TEXT(Format), ##__VA_ARGS__); \
	UE_LOG(LogImmutable, Warning, TEXT("%s: %s"), *FString(IMTBL_FUNCNAME), *_Msg) \
}
...
{ \
	const FString _Msg = FString::Printf(TEXT(Format), ##__VA_ARGS__); \
	UE_LOG(LogImmutable, Error, TEXT("%s: %s"), *FString(IMTBL_FUNCNAME), *_Msg) \
}
